...
update_dependencies() {
    ...
    _upgrade_output=$(sudo LC_ALL=C DEBIAN_FRONTEND=noninteractive apt-get -o Dpkg::Options::="--force-confold" upgrade --with-new-pkgs -y 2>&1) || _upgrade_status=$?
    printf '%s\n' "${_upgrade_output}"
    ...
    if ! sudo DEBIAN_FRONTEND=noninteractive apt-get autoremove -y; then
...
setup_simple_english_skill() {
    ...
    print_success "Simple English installed/updated for Claude Code, Codex, Gemini CLI, opencode, and Pi through the shared skill path."
    print_debug "${_install_output}"
}
...
# Matt Pocock skills to install in the shared Codex/Pi path and for opencode.
matt_pocock_skills() {
    printf '%s\n' \
...
# Install/update Matt Pocock engineering skills for Codex, Pi, and opencode.
setup_matt_pocock_skills() {
    ...
    local _output=""
    local _source_path=""
    local _args=(--yes skills@latest add "${_repo}" --global --agent codex --agent opencode --copy --yes)
    local _validation_dirs=("${_codex_skills_dir}" "${HOME}/.config/opencode/skills")
    local _missing=()
    ...
    if ! command -v npx &> /dev/null; then
        ...
        print_debug "Install Node.js >=22.20, then run: npx --yes skills@latest add mattpocock/skills --global --agent codex --agent opencode --copy --yes"
        return 1
    fi
    ...
    print_success "Matt Pocock skills installed/updated for Pi and Codex through the shared path, and for opencode."
    print_debug "${_output}"
}
...
enable_tmux_service() {
    ...
    # Use the same canonical D-Bus and machined-mediated fallback as Paseo so
    # lingering user managers remain reachable from non-login setup sessions.
    if ! paseo_systemctl_user daemon-reload; then
        print_warning "Could not reload the user service manager; skipping tmux service setup."
        return 1
    fi

    # Enable for next boot (never restart — restarting kills existing tmux windows)
    if paseo_systemctl_user enable tmux.service; then
        print_success "tmux service enabled."
    elif paseo_systemctl_user is-enabled tmux.service; then
    ...
    if ! paseo_systemctl_user is-active tmux.service; then
        if paseo_systemctl_user start tmux.service; then
...
run_setup_tasks() {
    ...
    echo -e "${GRAY}Version 231 | Last changed: Remove leftover Compound Engineering lfg skill and install manifest"
