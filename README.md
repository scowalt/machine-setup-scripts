...
Every setup run makes [Attention-kind](https://github.com/alexgreensh/attention-span) the active output style for Claude Code, Codex, Gemini CLI, and Pi on personal and work machines. Setup prefers the latest validated upstream style, keeps a valid installed copy when refresh fails, and falls back to the reviewed v0.7 copy in this repository. It writes Claude's native output style and setting, plus managed instruction blocks for Codex, Gemini CLI, and Pi. `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, and `PI_CODING_AGENT_DIR` select custom harness locations. Attention-kind is required and has no setup opt-out. Start a new session or run Claude `/clear`, Gemini `/memory reload`, or Pi `/reload` after an update.

Every setup run installs the latest [Simple English](https://github.com/AminBlg/SimpleEnglish) skill globally for Claude Code, Codex, Gemini CLI, and Pi. The installation is non-interactive and uses independent copies for cross-platform compatibility. `CLAUDE_CONFIG_DIR`, `CODEX_HOME`, and `PI_CODING_AGENT_DIR` select custom harness locations when they are set. Simple English is required on personal and work machines and has no setup opt-out.
