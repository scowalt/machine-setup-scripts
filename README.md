...
The setup scripts install/update Claude Code CLI, Gemini CLI, Codex CLI, Pi, RTK, tintinweb Pi subagents, Pi MCP adapter, Pi goal/autoresearch extensions, and Compound Engineering resources for supported agents. Personal machines also install Matt Pocock Pi skills; work machines also install Google Cloud CLI. Set `WORK_MACHINE=1` in `~/.env.local` for work machines. Set `BAN_CLAUDE_CODE=1` to skip Claude Code CLI setup. Set `BAN_RTK=1` to skip RTK setup. Set `BAN_COMPOUND=1` or `BAN_COMPOUND_PLUGIN=1` to skip Compound Engineering setup. Set `BAN_PI_SUBAGENTS=1` to keep the tintinweb Pi subagents extension inactive. Set `BAN_PI_MCP_ADAPTER=1` to keep the Pi MCP adapter inactive. Set `BAN_PI_GOAL_AUTORESEARCH=1` to keep the Pi goal/autoresearch extensions inactive. Set `BAN_MATT_POCOCK_SKILLS=1` to keep Matt Pocock Pi skills inactive.
