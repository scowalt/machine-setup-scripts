...
- AI agents and developer CLIs: Notion CLI (`ntn`), Claude Code CLI, Gemini CLI, Codex CLI, opencode, Pi coding agent, RTK token optimizer
- Agent skills/plugins: Setup always installs the latest Simple English skill globally for Claude Code, Codex, Gemini CLI, opencode, and Pi.
- Matt Pocock skills: Setup installs the managed engineering skills for Pi, Codex, and opencode on personal and work machines.
- Skill paths: Codex reads skills from `~/.agents/skills`. Gemini CLI shares the Simple English copy in this directory. opencode reads skills from `~/.config/opencode/skills`. Other harnesses use supported custom directories.
- opencode: Setup installs the `anomalyco/tap/opencode` Homebrew formula where brew exists (mac.sh, wsl.sh, bazzite.sh, and ubuntu.sh when brew is on PATH). Otherwise setup uses the official `https://opencode.ai/install` installer with `--no-modify-path`, which keeps `opencode` in `~/.opencode/bin` and leaves shell rc files untouched because chezmoi owns shell configuration. Setup removes a legacy Bun `opencode-ai` package and a standalone binary when migrating to the formula. Setup does not seed `~/.config/opencode/opencode.json`; chezmoi owns that file. Setup only warns when `SYNTHETIC_API_KEY` is unset (Kimi K3 is the opencode default) and, on `WORK_MACHINE=1` machines, when `ZAI_API_KEY` is unset (GLM-5.3 with the z.ai Coding Plan base URL is the work default). Setup also installs the Simple English and Matt Pocock skills for opencode via the skills CLI and configures RTK with `rtk init -g --opencode`. Windows machines get opencode through WSL, so win.ps1 stays untouched; pi.sh deliberately installs no AI agents and stays untouched.
...
- RTK: Setup installs RTK on all machines. Setup initializes RTK for supported agents when RTK is available.
- Work machines: Setup installs Google Cloud CLI. Setup updates its components when the component manager is available.
- Opt-outs: Set `BAN_CLAUDE_CODE=1` to skip Claude Code CLI. Set `BAN_OPENCODE=1` to skip opencode. Set `BAN_MATT_POCOCK_SKILLS=1` to remove the managed Matt Pocock skills and keep them inactive. The older `BAN_MATT_POCKOCK_SKILLS=1` spelling also works. `WORK_MACHINE=1` does not disable these skills. Set `BAN_RTK=1` in `~/.env.local` to skip RTK.
