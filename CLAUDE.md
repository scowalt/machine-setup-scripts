...
- Agent skills/plugins: Setup always installs the latest Simple English and HumanLayer `show-me` skills globally for Claude Code, Codex, Gemini CLI, and Pi.
- Matt Pocock skills: Setup installs the managed engineering skills for Pi and Codex on personal and work machines.
- Skill paths: Pi and Codex share canonical managed agent skills in `~/.agents/skills`. Gemini CLI shares the Simple English and `show-me` copies in this directory. Setup excludes obsolete direct Pi copies and package-skill collisions without deleting intentional shared copies.
...
- Opt-outs: Set `BAN_CLAUDE_CODE=1` to skip Claude Code CLI. Set `BAN_MATT_POCOCK_SKILLS=1` to remove the managed Matt Pocock skills and keep them inactive. The older `BAN_MATT_POCKOCK_SKILLS=1` spelling also works. `WORK_MACHINE=1` does not disable these skills. Simple English and `show-me` have no opt-out.
