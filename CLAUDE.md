...
- Skill paths: Pi and Codex share canonical setup-managed skills in `~/.agents/skills`. Gemini CLI shares the Simple English copy in this directory. opencode reads skills from `~/.config/opencode/skills`. Setup excludes obsolete direct Pi copies and package-skill collisions without deleting intentional shared copies.
