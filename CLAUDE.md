...
- Agent styles/skills/plugins: Attention-kind is required and always active for Claude Code, Codex, Gemini CLI, and Pi on personal and work machines. Setup refreshes the latest validated upstream style, falls back to an installed or reviewed v0.7 copy, preserves unrelated instruction content with managed markers, and has no opt-out. The latest Simple English skill is always installed globally with copied files for the same four agents; Codex and Gemini CLI share the canonical `~/.agents/skills/simple-english` copy. Setup respects custom harness directories where the harness supports them, validates installed artifacts, and treats both features as required. Legacy setup-managed Impeccable skill copies and Cursor subagent files are removed without touching project-scoped data or hooks. Matt Pocock engineering skills are installed for Pi on personal machines. Notion CLI is installed with its native installer on macOS/Linux and WinGet on Windows; setup updates the CLI but leaves `ntn login` and shell completions to the user. Claude Code CLI is installed with Anthropic's native installer, not npm/Bun, so it does not depend on a global Node runtime. RTK is installed for all machines and initialized for supported agents when available. Work machines install Google Cloud CLI and update its components when the component manager is available. Skip Claude Code CLI with `BAN_CLAUDE_CODE=1`; skip Matt Pocock skills with `WORK_MACHINE=1` or `BAN_MATT_POCOCK_SKILLS=1`; skip RTK with `BAN_RTK=1` in `~/.env.local`
