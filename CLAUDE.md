...
- Skill paths: Codex reads skills from `~/.agents/skills`. Gemini CLI shares the Simple English copy in this directory. Other harnesses use supported custom directories.
...
- Opt-outs: Set `BAN_CLAUDE_CODE=1` to skip Claude Code CLI. Set `BAN_MATT_POCOCK_SKILLS=1` to remove the managed Matt Pocock skills and keep them inactive. The older `BAN_MATT_POCKOCK_SKILLS=1` spelling also works. `WORK_MACHINE=1` does not disable these skills. Set `BAN_RTK=1` in `~/.env.local` to skip RTK.
