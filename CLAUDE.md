...
- Pi output style: Setup installs or updates unpinned `npm:pi-prose` on every machine with no opt-out. It seeds `${PI_CODING_AGENT_DIR:-~/.pi/agent}/prose/config.json` with the `matter-of-fact` user default only when the file is absent. Existing files remain unchanged, including explicit `default`, empty, and malformed configurations. The dotfiles repository keeps `pi-prose` in Pi's package list and uses a Chezmoi `create_` source file for the same initial default.
