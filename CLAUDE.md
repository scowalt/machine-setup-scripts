...
### OpenCode Go and managed Muse profiles

- All six scripts add Go access for Pi and the selectable Muse Spark 1.3 Contributor/xhigh Paseo profile on personal and work machines. Preserve GPT-6 Astra defaults. Do not reinstall the retired OpenCode CLI or substitute paid Zen.
...
- Coordinate auth writes with Pi's native credential lock. Reject malformed or linked metadata, protect local secret permissions and Windows ACLs, and never print keys or arbitrary exception output. Keep the embedded credential helper identical in all six scripts. Use the installed built-in Go catalog and native `xhigh` mapping without an inference request. Reject explicit active `opencode-go` model/provider overrides without rewriting them. Trust the already-installed Pi code under its normal npm umask, but require private credentials and non-writable credential directory boundaries. Set `PI_GO_LOCK_MODULE` to Pi's installed `proper-lockfile/index.js` for the additional offline native-lock/catalog fixtures. Failed Go validation blocks later Pi package operations and profile/managed-daemon setup; continue unrelated work and report the failed result. Go diagnostics must carry a controlled operation label and allowlisted reason or native error code through every Bash/PowerShell wrapper. Reject unrecognized helper output and never echo raw stderr, arbitrary exceptions, credentials, or custom paths. Preserve the failing operation across cleanup, and report unknown causes without guessing.
- The managed Paseo profile belongs in `<PASEO_HOME>/config.json` under `daemon.agentProfiles`, not Electron settings. Repair the managed provider/model/reasoning fields, recreate deletion, and preserve optional customizations and all other profiles. A missing credential does not block profile creation. Keep the shared embedded profile helper identical. Custom `PASEO_HOME` directories must already exist, be private and account-owned, and lie below the account HOME without links. Reject empty, relative, HOME-itself, and outside-HOME overrides rather than silently selecting another profile. Native PID reservations exclude daemon startup, not arbitrary external editors. A verified custom-home transaction must still block the later legacy managed-daemon installer, which assumes the default home; preserve that owner's launch environment. Validate Windows metadata/ancestor ACLs and use only native PowerShell modules for inspection and ACL preservation.
- Native Linux `HEADLESS=1` with the default Paseo home can recover the known group-writable native PID and systemd parent directories. Preflight all candidates and prove the running setup-managed owner before any permission change. Repair only account-owned real `~/.paseo/paseo.pid`, `~/.config`, `~/.config/systemd`, and `~/.config/systemd/user` through stable descriptor-relative handles. PID repair requires a private Paseo home. Remove write bits without changing owners, contents, HOME, or unrelated paths; never recurse. Keep world-writable, linked, foreign-owned, custom-home, Desktop/self-hosted, stopped/unverified-service, and drop-in cases blocked. `verify-owner` remains read-only. Use nonblocking leaf opens and descriptor checks so a FIFO replacement cannot hang inspection. Managed wrappers use `umask 077`; exact legacy/new wrapper and cleanup-provenance recognition must remain safe. On the intermediate Muse restart through a legacy wrapper, wait for a complete native PID, re-prove its owner and secure that inode before reporting restoration. Missing/partial PID readiness is bounded and failure must not start a second daemon. Test observed permissions, unchanged profiles, native PID creation, delayed/partial/missing PID, path/FIFO swaps, failures and platform gates using temporary fixtures only.
- Setup can restart an identified, setup-managed local daemon when a profile change requires it. Preserve exact headless gates, the macOS canary, and Windows/WSL headless rejection. Verify ownership, home selection, and a stopped interval before an offline merge. Restore the owning service after failure. Defer with manual instructions when Desktop owns a running daemon, ownership is uncertain, or setup runs inside the daemon it must stop. Service drop-ins and environment files also require manually stopped synchronization. Never kill Desktop, create a replacement daemon, or change remote hosts. Set `PASEO_MUSE_PID_LOCK_MODULE` to Paseo 0.8.0's `dist/src/server/pid-lock.js` for temporary native startup-exclusion fixtures without a daemon.
...
- Run `bash tests/pi-opencode-go-contract.sh`, `bash tests/paseo-muse-profile-contract.sh`, and `bash tests/opencode-go-wiring-contract.sh`, with `PWSH_BIN` for PowerShell wrappers, plus affected AI-agent, model-default, headless, release-channel, shared-runtime, and Telegram contracts. Extract only tested functions and use temporary homes/mock lifecycle commands. Never run live setup or fleet changes during development.

### Pi profile permissions and surplus Paseo CLIs

- All six scripts prepare the default and explicitly selected Pi profile directories after dotfiles and before Pi mutations. Keep the embedded permission helper identical. Preflight both profiles, repair only account-owned real directories inside HOME to `0700` or equivalent private native Windows ACLs, and preserve HOME, unrelated ancestors, file contents, and credential permissions. Do not recurse or follow linked profiles. Preserve the trusted Linux system HOME alias. Permission failure blocks later Pi mutations and dependent Muse/daemon setup, while unrelated work continues. Unix leaf creation uses isolated `/usr/bin/python3` descriptor-relative operations and fails closed when unavailable. Windows creation and ACL changes use pinned native handles and stable volume/file identity.
- Plain selects a metadata-verified compatible CLI before configuration-sensitive status calls. Prefer an explicitly validated CLI or the existing Bun global CLI over PATH. Do not fall back from an incompatible verified managed release to an older PATH copy. Keep all six Plain retirement helpers and the shared CLI identity helper copies identical.
- Surplus CLI cleanup runs only after eligible headless setup validates the retained Bun CLI and managed daemon health. Limit candidates to official account-owned npm installs under `~/.local` and standard mise Node global prefixes. Preserve custom/project/source/Desktop/system installations, unrelated packages, and all daemon/plugin state. Validate all candidates before removal and preserve active or stopped service references and uncertain process owners. Cleanup adds no independent stop/restart operation and does not bypass custom-home, headless, channel, or macOS-canary gates. Run the shared Muse helper's read-only `verify-owner` mode before headless mutations, even when profile data is unchanged. Bind the generated wrapper's `PASEO_SETUP_CLI` launch marker to its exact exec command. Cleanup must inspect process launch origin, cwd, runtime environment, and loaded service ownership before exempting native process titles.
- Use native npm with offline mode and lifecycle scripts disabled for verified surplus removal. Never relax npm policy, recursively inventory HOME, remove an in-use CLI, or use cleanup as a reason to reset configuration. Keep diagnostics controlled and secret-free. Run `tests/pi-profile-permissions-contract.sh`, `tests/paseo-cli-cleanup-contract.sh`, and affected Go/package/runtime/Plain/headless/Muse/channel suites with available `PWSH_BIN`. Use only extracted helpers and temporary fixtures. Native Windows ACL behavior still needs Windows verification.
...
- All six scripts retire `paseo-plain` on future setup runs through the verified local daemon's native removal command, including disabled, pinned, and directory registrations under that exact ID. Keep the embedded Node helpers identical. No installation, update, migration, plugin enablement, daemon lifecycle operation, or model request is part of retirement.
- Preserve unrelated plugins/configuration, credentials, projects, external sources, `plugin-data/paseo-plain`, and recovery backups. Native removal deletes the managed checkout and `plugin-settings/paseo-plain`; back up native settings privately under `setup-recovery/paseo-plain-retirement` first. Reject shared deletion targets, linked/malformed metadata, unfinished migration journals, and occupied retirement backups rather than overwriting them. See README retirement/recovery instructions.
- Use `PASEO_HOME` or the default only when unset. Reject empty, relative, outside-HOME, HOME-itself, and linked overrides; preserve the trusted Linux system HOME alias. Set the resolved home in CLI children, clear inherited `PASEO_HOST`, and pass an explicit loopback `--host` for plugin commands. Paseo has no global `--home` option.
- Retirement may read a POSIX native PID with exact mode `0664` only when it is account-owned, regular, single-link, and protected by a private Paseo home and trusted non-writable ancestors (including the root-owned sticky temporary-directory exception). Keep this exception PID-only and read-only: preserve permissions, service drop-ins and launch environments, and leave managed-daemon ownership/repair restrictions unchanged. Bound reads, reject links/FIFOs and changed inode/home/contents before CLI commands, and allow heartbeat timestamp updates. Use controlled `pid preflight` diagnostics. Test the actual native PID writer plus the active directory-plugin/no-store/no-settings case.
- Compatible Paseo CLI/daemon 0.8.x and Node >=22.19 are required for removal; Pi and global plugin enablement are not. Absent registrations are an offline no-op. Unavailable/unsafe retirement returns failure while unrelated setup continues, with controlled operation/reason diagnostics and manual instructions. Keep raw command output, arbitrary exceptions, preferences, and credentials out of logs. Preserve channel and platform gates.
- Test only extracted helpers in temporary homes, never live setup or fleet changes. Run `tests/paseo-plain-setup-contract.sh` with `PWSH_BIN` for PowerShell wrappers and `PASEO_TEST_PLUGIN_SERVICE_MODULE` for the native retirement/fixture-isolation tests. `tests/paseo-plain-native-retirement.mjs` uses real source/config managers but inert plugin workers and no daemon. Native Windows ACLs still need Windows verification.
- Clear inherited `GIT_*` variables before constructing native fixtures; re-add test-owned values only. Isolate Git configuration/hooks, verify repository paths, and use per-command identity settings. No real plugin code or skills execute during tests.
