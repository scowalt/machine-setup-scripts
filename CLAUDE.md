...
- Agent skills/plugins: Compound Engineering is installed for Claude Code and Pi on personal machines; Matt Pocock engineering skills are installed for Pi on personal machines. Skip Compound with `WORK_MACHINE=1` or `BAN_COMPOUND_PLUGIN=1`; skip Matt Pocock skills with `WORK_MACHINE=1` or `BAN_MATT_POCOCK_SKILLS=1` in `~/.env.local`
