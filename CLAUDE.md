...
- AI agents and developer CLIs: Notion CLI (`ntn`), Claude Code CLI, Gemini CLI, Codex CLI, opencode, Pi coding agent
...
- Opt-outs: Set `BAN_CLAUDE_CODE=1` to skip Claude Code CLI. Set `BAN_OPENCODE=1` to skip opencode. Set `BAN_MATT_POCOCK_SKILLS=1` to remove the managed Matt Pocock skills and keep them inactive. The older `BAN_MATT_POCKOCK_SKILLS=1` spelling also works. `WORK_MACHINE=1` does not disable these skills.
