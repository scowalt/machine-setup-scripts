...
- Agent skills/plugins: Setup always installs the latest Simple English skill globally for Claude Code, Codex, Gemini CLI, and Pi. Codex and Gemini CLI share the canonical `~/.agents/skills/simple-english` copy. Setup uses custom harness directories where the harness supports them. Setup validates the installed files. The skill is required on personal and work machines. Setup removes legacy Impeccable skill copies and Cursor subagent files. It does not change project data or hooks. Personal machines also receive the Matt Pocock engineering skills for Pi.
...
- Opt-outs: Set `BAN_CLAUDE_CODE=1` to skip Claude Code CLI. Set `WORK_MACHINE=1` or `BAN_MATT_POCOCK_SKILLS=1` to skip the Matt Pocock skills. Set `BAN_RTK=1` in `~/.env.local` to skip RTK.
