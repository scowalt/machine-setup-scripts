...
- Agent skills/plugins: Setup always installs the latest Simple English, HumanLayer `show-me`, and PR Lens skills globally for Claude Code, Codex, Gemini CLI, and Pi.
- PR Lens: Setup requests `coldteadotai/pr-lens` with the specific skill `pr-lens` on each machine's next setup run. No remote fleet installation runs. Upstream contents and default hosted uploads remain unchanged, including on work machines. The hosted canvas receives the whole graph JSON and allows anyone with the view link to read it without login. Keep secret edit links out of logs and commits. Setup adds no approval gate, local-only policy, mandatory PR diagrams, hooks, GitHub App, global PR Lens CLI, provider credentials, or dotfiles override. Setup and offline tests do not render, upload, or post diagrams. Both managed copies must contain five nonempty regular files: `SKILL.md`, `LICENSE`, `references/graph-document.md`, `references/config.md`, and `references/example.graph.json`. Reject missing or empty files and symlink files or directories, including linked `references` directories. The skill invokes the CLI through `npx` on demand. PR attachments need `gh` >=2.99 and suitable write permissions. Windows, Linux ARM64, and ARM32 runtime workflows are not smoke-tested. See README.md for runtime limits.
- Matt Pocock skills: Setup installs the managed engineering skills for Pi and Codex on personal and work machines.
- Skill paths: Pi and Codex share canonical managed agent skills in `~/.agents/skills`. Gemini CLI shares the Simple English, `show-me`, and PR Lens copies in this directory. Setup excludes obsolete direct Pi copies and package-skill collisions without deleting intentional shared copies.
...
- Opt-outs: Set `BAN_MATT_POCOCK_SKILLS=1` to remove the managed Matt Pocock skills and keep them inactive. The older `BAN_MATT_POCKOCK_SKILLS=1` spelling also works. `WORK_MACHINE=1` does not disable these skills. Simple English, `show-me`, and PR Lens have no opt-out.
...
- Failure diagnostics use controlled operation labels and reason codes. Never print raw command output, arbitrary exception messages, preferences, or credentials. Preserve the migration recovery instructions and private backups. Do not infer a root cause from a generic failed command.
...
- Install/update through the correct local daemon's native Git plugin commands. Track `main` at `https://github.com/scowalt/paseo-plain.git`. Do not replace other sources, directory installs, pinned refs, or explicit disabled choices. Source-preservation warnings must distinguish directory/non-Git sources, repository mismatches, and custom/pinned refs through controlled labels, without printing source values. Direct the user to review the source before planning a migration.
- On future setup runs, migrate matching, enabled `release` installations to `main` under the same ID. Verify the native source record before migration. Paseo 0.8 cannot change a branch in place. The approved one-time migration uses native remove/add with verified private backups. Native removal deletes the managed checkout and `plugin-settings/paseo-plain`. Preserve `plugin-data/paseo-plain` and restore native settings before activation. Do not edit cached `sources.json` or restart the daemon.
- Keep the migration journal and backups in `<PASEO_HOME>/setup-recovery/paseo-plain-release-to-main`. Failures, timeouts, and incomplete journals stop automatic changes until reviewed. Do not turn this exception into generic remove/reinstall recovery. Never move a backup that is serving as an installed directory source.
...
- Clear inherited `GIT_*` variables before building a Git fixture environment. Re-add only test-owned values, isolate Git configuration and hooks, and verify the repository path. Use per-command `-c` values for fixture identity. Run `tests/test_paseo_native_fixture_isolation.py` to prove hook variables cannot redirect tests into the caller's repository.
- Future setup runs only: no fleet inventory, SSH rollout, or live migration during development. Do not source full setup scripts in new tests: some platforms call main unconditionally. Extract only the tested function or embedded installer. Test native migration behavior with `tests/paseo-plain-native-migration.mjs` against an explicitly supplied Paseo module and temporary fixture homes.
