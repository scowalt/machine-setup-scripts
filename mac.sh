...
configure_fnm() {
    ...
    if [ -d ~/.config/fish/conf.d ]; then
        ...
        if ! grep -q "fnm env" ~/.config/fish/conf.d/fnm.fish 2>/dev/null; then
            print_message "Configuring fnm for Fish shell..."
            cat <<'EOF' > ~/.config/fish/conf.d/fnm.fish
# auto-generated by setup script
fnm env | source
EOF
            print_success "fnm configured for Fish shell."
        else
            print_warning "fnm Fish configuration already exists."
        fi
    fi
}

# Configure pyenv for Fish shell
configure_pyenv() {
    # Set up Fish shell integration for pyenv
    if [ -d ~/.config/fish/conf.d ]; then
        # only add once
        if ! grep -q "pyenv init" ~/.config/fish/conf.d/pyenv.fish 2>/dev/null; then
            print_message "Configuring pyenv for Fish shell..."
            cat <<'EOF' > ~/.config/fish/conf.d/pyenv.fish
# auto-generated by setup script
# pyenv installed via Homebrew, so no need to set PYENV_ROOT or modify PATH
if command -v pyenv 1>/dev/null 2>&1
    pyenv init - | source
end
EOF
            print_success "pyenv configured for Fish shell."
        else
            print_warning "pyenv Fish configuration already exists."
        fi
    fi
    
    # Add to bash/zsh for compatibility
