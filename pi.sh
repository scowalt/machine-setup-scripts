...
EOF
    ...
    if ! pyenv versions --bare &>/dev/null || [ -z "$(pyenv versions --bare 2>/dev/null)" ]; then
        print_message "Installing Python 3.12 as default version (this will take a while on Raspberry Pi)..."
        
        # Check available memory before compiling Python
        local total_mem=$(free -m | awk '/^Mem:/{print $2}')
...
# Main execution
print_message "Starting Raspberry Pi setup script v8..."
