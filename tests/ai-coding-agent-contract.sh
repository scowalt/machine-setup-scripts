...
for file in "${bash_setup_scripts[@]}"; do
    ...
    assert_contains "${file}" '^[[:space:]]+setup_pi_companion_packages \|\| _setup_had_errors=1$' 'Pi companion package main wiring'
    assert_order "${file}" '^[[:space:]]+setup_pi_claude_bridge \|\| _setup_had_errors=1$' '^[[:space:]]+setup_pi_companion_packages \|\| _setup_had_errors=1$' 'Pi companion package setup after Pi Claude bridge'
...
assert_contains win.ps1 '^[[:space:]]+if \(-not \(Setup-PiClaudeBridge\)\) \{ [$]piSetupFailed = [$]true \}$' 'Pi Claude bridge main wiring'
assert_order win.ps1 '^[[:space:]]+Install-CodexCli$' '^[[:space:]]+Remove-RtkResources$' 'Codex CLI installed before retired RTK cleanup'
assert_order win.ps1 '^[[:space:]]+elseif \(Install-PiCli\) \{$' '^[[:space:]]+if \(-not \(Setup-PiClaudeBridge\)\) \{ [$]piSetupFailed = [$]true \}$' 'Pi installed before Pi Claude bridge'
...
assert_contains win.ps1 '^[[:space:]]+if \(-not \(Setup-PiMcpAdapter\)\) \{ [$]piSetupFailed = [$]true \}$' 'Pi MCP adapter main wiring'
...
assert_contains win.ps1 '^[[:space:]]+if \(-not \(Setup-PiCompanionPackages\)\) \{ [$]piSetupFailed = [$]true \}$' 'Pi companion package main wiring'
assert_order win.ps1 '^[[:space:]]+if \(-not \(Setup-PiClaudeBridge\)\) \{ [$]piSetupFailed = [$]true \}$' '^[[:space:]]+if \(-not \(Setup-PiCompanionPackages\)\) \{ [$]piSetupFailed = [$]true \}$' 'Pi companion package setup after Pi Claude bridge'
