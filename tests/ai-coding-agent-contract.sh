...
for file in "${bash_setup_scripts[@]}"; do
    ...
    assert_contains "${file}" '^setup_pi_ask_user\(\)' 'Pi Ask User setup function'
    assert_contains "${file}" 'npm:pi-ask-user' 'Pi Ask User package source'
    assert_contains "${file}" '^[[:space:]]+setup_pi_ask_user$' 'Pi Ask User main wiring'
    assert_order "${file}" '^[[:space:]]+setup_pi_claude_bridge$' '^[[:space:]]+setup_pi_ask_user$' 'Pi Ask User setup after Pi Claude bridge'
...
assert_contains win.ps1 'function Setup-PiAskUser' 'Pi Ask User setup function'
assert_contains win.ps1 'npm:pi-ask-user' 'Pi Ask User package source'
assert_contains win.ps1 '^[[:space:]]+Setup-PiAskUser$' 'Pi Ask User main wiring'
assert_order win.ps1 '^[[:space:]]+Setup-PiClaudeBridge$' '^[[:space:]]+Setup-PiAskUser$' 'Pi Ask User setup after Pi Claude bridge'
