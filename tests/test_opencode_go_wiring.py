...
class WiringTests(unittest.TestCase):
    def bash_block(self, name):
        ...
        begin = main.index("    if ! remove_pi_prose; then")
...
'''
    ...
    def test_failures_are_aggregated_and_deferred_updates_do_not_restart(self):
        for name in BASH:
            for scenario in ("retirement-failure", "pi-failure", "go-failure",
                ...
                with self.subTest(script=name, scenario=scenario):
                    ...
                    ready = 0 if scenario in ("retirement-failure", "pi-failure", "go-failure") else 1
                    ...
                    if not ready or scenario in ("deferred", "profile-failure"):
                        self.assertNotIn("daemon", calls)
                    if scenario in ("retirement-failure", "pi-failure"):
    ...
    def test_powershell_wiring_and_failure_aggregation(self):
        ...
        block = main[main.index("    if (-not (Remove-PiProse))"):
                     main.index("    if (-not (Install-SimpleEnglishSkill))")]
        inert = ("Set-PiDefaults", "Remove-PiSyntheticModels", "Seed-PiZaiModels",
...
'''
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fixture.write_text(code)
            for scenario in ("success", "retirement-failure", "pi-failure", "go-failure",
