...
- Supported strict headless targets are native Linux `ubuntu.sh`, `pi.sh`, and `bazzite.sh`. `mac.sh` should expose a canary-only LaunchDaemon path until cold-boot/no-login evidence proves it. `wsl.sh` and `win.ps1` should fail early when `HEADLESS=1` because they cannot currently guarantee a daemon after host reboot without interactive login.
...
- `ubuntu.sh`: existing `HEADLESS=1` handling in `detect_machine_type()` establishes headless as a hard remote-operability signal; unrestricted passwordless sudo remains behind the separate `HEADLESS_PASSWORDLESS_SUDO=1` opt-in.
...
- `ubuntu.sh`, `pi.sh`, `bazzite.sh`, `wsl.sh`: `enable_user_lingering()` and `install_ccgram()` provide the closest systemd-user patterns, but their warning/skip behavior must be tightened for Paseo.
...
**Patterns to follow:**
- `ubuntu.sh` `detect_machine_type()` for early headless contract handling without granting passwordless sudo by default.
...
**Patterns to follow:**
- `enable_user_lingering()` in `ubuntu.sh`, `pi.sh`, and `bazzite.sh`, but with strict failure semantics.
...
- Recommended implementation validation:
  - `bash -n mac.sh ubuntu.sh pi.sh bazzite.sh wsl.sh`
  - `bunx shellcheck mac.sh ubuntu.sh pi.sh bazzite.sh wsl.sh`
...
- Related code: `mac.sh`, `ubuntu.sh`, `pi.sh`, `bazzite.sh`, `wsl.sh`, `win.ps1`, `lefthook.yml`
